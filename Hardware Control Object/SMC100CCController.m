classdef SMC100CCController < handle
        ...
        function obj = SMC100CCController(com,address)
            ...
            try
                ...
                while ~strcmp(obj.controllerState, 'READY')
                    ...
                    warning off;
                    obj.Error_and_State()
                    while strcmp(obj.controllerState, 'HOMING')
                        pause(0.5);
                        obj.Error_and_State()
                    end
                    warning on;
                ...
                if ~strcmp(obj.controllerState, 'READY')
                    ...
                    obj.status = true;
                    disp('MFA-CC is Connected.')
                end
                obj.sendCommand('SL','?');
                obj.L_lim = obj.feedbackData(4:end);
                obj.sendCommand('SR','?');
                obj.R_lim = obj.feedbackData(4:end);
            catch error
    ...
    methods
        ...
        function Move(self, value, mode)
            ...
            %      mode : 'rel'           -> relative angle
            %           : 'abs'(default)  -> absolute angle
            if (nargin < 3)
                mode = 'abs';
            end
            if ((value < str2double(self.L_lim)) || (value > str2double(self.R_lim)))
                warning(strcat('Value is out of range ! (from ',self.L_lim,' to ',self.R_lim,' )'));
            else
                switch mode
                    case 'abs'
    ...
    methods(Access = private)
        % Read feedback from Tanlian Stage
